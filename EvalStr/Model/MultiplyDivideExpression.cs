...
{
    public class MultiplyDivideExpression<T> : ArithmeticExpression<T> where T : ICalculator
    {
        public override T GetValue()
        {
            if (this.Operators.Count > 0 && this.Operands.Count > this.Operators.Count)
            {
                ...
                for (int i = 0; i < this.Operators.Count; i++)
                {
                    ...
                    if (this.Operators[i].Operator == OperatorType.Multiply)
                    {
                        value = (T)value.Multiply(op);
                    }
                    else
                    {
                        value = (T)value.Divide(op);
                    }
                }
                return value;
            }
        ...
        {
            return OperatorType.Multiply.Priority();
        }
    }
}
